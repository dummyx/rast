...
Targeted for SVT-AV1 v4.0.1 encoder API. The goal is ergonomics with RAII for initialization/teardown while keeping API parity close to the C interface.

Features
- `encoder` feature (default) mirrors the underlying sys crate.
- `decoder` feature is available but not enabled by default; decoder headers are not vendored.
