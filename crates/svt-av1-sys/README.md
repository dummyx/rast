...
Raw Rust FFI bindings to the SVT-AV1 C API (encoder; decoder optional), generated with bindgen at build time. Targeted for SVT-AV1 v4.0.1 (vendored copy exposes encoder headers).
...
- Feature flags: `encoder` and `decoder` enable their respective APIs (only `encoder` is enabled by default; the vendored tree is encoder-only).
...
- `SVT_AV1_ENABLE_LTO=0/1`: toggle link-time optimization when building the vendored copy (default off; enabling may require a non-`rust-lld` toolchain).
